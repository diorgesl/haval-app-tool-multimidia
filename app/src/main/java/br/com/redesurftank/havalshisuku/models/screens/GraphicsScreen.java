...
public class GraphicsScreen implements Screen {
    ...
    public static class GraphOptions {
        ...
        private static final String[] graphsValueMap = { EV_CONSUMPTION, GAS_CONSUMPTION, HEV_CONSUMPTION,
                ENERGY_EFFICIENCY, CAR_SPEED };
    }
        ...
        switch (key) {
            case UP:
                ...
                if (currentGraphIndex < 0)
                    currentGraphIndex = GraphOptions.graphsValueMap.length - 1;
                break;
            case ENTER:
            case DOWN:
                currentGraphIndex++;
                if (currentGraphIndex >= GraphOptions.graphsValueMap.length)
                    currentGraphIndex = 0;
                break;
            case BACK:
